...
int main() {    
    ...
    std::cout << "Number128:      " << pb.toString() << std::endl;
    ...
    float rightMin = 0;
    float rightMax = 0;
    left->getMinimumFloatRange(leftMin, leftMax);
    right->getMinimumFloatRange(rightMin, rightMax);
    std::cout << "Minimum range:  [(" << leftMin << " .. " << leftMax << "); (" << rightMin << " .. " << rightMax << ")]" << std::endl;
    


    
    // compress
    Postbinary::Package* compressedPb128 = Postbinary::Utilities::Compressor::compress(pb, 12);
    std::cout << "Compressed: " << compressedPb128->toString() << std::endl;

    Postbinary::Pb128_32ip* pbDecompressed = Postbinary::Utilities::Compressor::decompressAsPb128(*compressedPb128);
    std::cout << "Decompr:        " << pbDecompressed->toString() << std::endl;


    Postbinary::Pb128_32ip& postbinaryNumber = *pbDecompressed;
    left = postbinaryNumber.getLeft();
    std::cout << "New Left:       " << left->toString() << std::endl;

    right = postbinaryNumber.getRight();
    std::cout << "New Right:      " << right->toString() << std::endl;

    left->getMinimumFloatRange(leftMin, leftMax);
    right->getMinimumFloatRange(rightMin, rightMax);
    std::cout << "New min range:  [(" << leftMin << " .. " << leftMax << "); (" << rightMin << " .. " << rightMax << ")]" << std::endl;
...
void test(float& left, float& right) {
    ...
    // compress
    std::cout << "Number :   " << postbinaryNumber.toString() << std::endl;
    Postbinary::Package* package = Postbinary::Utilities::Compressor::compress(postbinaryNumber, 10);
    std::cout << "Package: " << package->toString() << std::endl;


    Postbinary::Pb64_32p* pbDecompressed = Postbinary::Utilities::Compressor::decompressAsPb64(*package);
    std::cout << "Decompr:   " << pbDecompressed->toString() << std::endl;

    postbinaryNumber = *pbDecompressed;
